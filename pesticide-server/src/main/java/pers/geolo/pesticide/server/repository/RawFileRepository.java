...
import pers.geolo.pesticide.server.exception.ConfigurationException;
...
public class RawFileRepository implements FileRepository {
    ...
    public FileInfo getFile(String id) throws IOException {
        ...
        if (!file.exists()) {
            throw new pers.geolo.pesticide.server.exception.FileNotFoundException("the file of path = " + id + " is not found.");
        }
        return new FileInfo() {
            @Override
            public String getId() {
